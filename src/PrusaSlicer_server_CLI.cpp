...
//    sla_print_config.apply(m_print_config, true);
    ...
    for (auto const &opt_key : m_actions) {
        if (opt_key == "help") {
        ...
        } else if (opt_key == "server_port") {
		
			const ConfigOptionInt *opt_serverport = m_config.opt<ConfigOptionInt>(opt_key);
			printf("server-port is %ld\n", (long) opt_serverport->getInt());
			runServer(opt_serverport->getInt());
            ...
            for (Model &model_in : m_models) {
                ...
                {
                    if(s.percent >= 0) {// FIXME: is this sufficient?
                        printf("%3d%s %s\n", s.percent, "% =>", s.text.c_str()); fflush(stdout);
						size_t size = snprintf(nullptr, 0, "%3d%% => %s", s.percent, s.text.c_str())+1;
						std::unique_ptr<char[]> buf(new char[size]);
						snprintf(buf.get(), size, "%3d%% => %s", s.percent, s.text.c_str());
						sendToClient( buf.get() );
					}
                });
