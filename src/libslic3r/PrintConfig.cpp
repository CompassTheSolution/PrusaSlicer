...
PrintConfigDef::PrintConfigDef()
{
	...
	this->init_layer_pattern_params();
...
std::string FullPrintConfig::validate()
{
	...
	if (layer_pattern_enabled && layer_pattern_count > 0)	// If we're actually using layer patterns
	{
		if (layer_pattern_minimum_z < 0.0)
			return std::string("layer_pattern_minimum_z must be > 0.");
		if (layer_pattern_maximum_z < layer_pattern_minimum_z)
			return std::string("layer_pattern_maximum_z must be > layer_pattern_minimum_z");

		// Make sure we have enough values for the per-layer parameters
		if (layer_pattern_perimeters.values.size() < layer_pattern_count)
			return std::string("Too few layer_pattern_perimeters");
		if (layer_pattern_fill_pattern.values.size() < layer_pattern_count)
			return std::string("Too few layer_pattern_fill_pattern");
		if (layer_pattern_connector_type.values.size() < layer_pattern_count)
			return std::string("Too few layer_pattern_connector_type");
		if (layer_pattern_fill_density.values.size() < layer_pattern_count)
			return std::string("Too few layer_pattern_fill_density");
		if (layer_pattern_shift_distance.values.size() < layer_pattern_count)
			return std::string("Too few layer_pattern_shift_distance");
		if (layer_pattern_fill_angle.values.size() < layer_pattern_count)
			return std::string("Too few layer_pattern_fill_angle");
		if (layer_pattern_angle_relative.values.size() < layer_pattern_count)
		...
		if (layer_pattern_line_gap.values.size() < layer_pattern_count)
			return std::string("Too few layer_pattern_line_gap");

		// Now validate each one of the individual parameters
		for (int l = 0; l < layer_pattern_count; l++)
		{
			if (layer_pattern_perimeters.values[l] < 0)
				return std::string("Invalid value for layer_pattern_perimeters");
//FIXME More validation should be done here!
		}
#ifdef FOR_REFERENCE_ONLY
		// From Printconfig.hpp
		ConfigOptionBool		layer_pattern_enabled;
		ConfigOptionBool		layer_pattern_continuous_print;
		ConfigOptionBool		layer_pattern_avoid_flyover;
		...
		ConfigOptionFloats					layer_pattern_line_gap;	// Again, I know it's not proper pluralization
#endif
	}
